...
const FourtwentyBlockTracker = require('fourtwenty-block-tracker')
const FourtwentyQuery = require('fourtwozerojs-query')
const JsonRpcEngine = require('json-rpc-engine')
const providerAsMiddleware = require('fourtwenty-json-rpc-middleware/providerAsMiddleware')
...
function createTestSetup () {
  ...
  const blockTracker = new FourtwentyBlockTracker({
...
function createSubGenerator({ subType, provider }) {
  return pify(function() {
    ...
    provider.sendAsync({ method: 'fourtwenty_subscribe', params: args }, (err, res) => {
...
function createNewSub({ id, provider }) {
  ...
  provider.on('data', (_, message) => {
    if (message.method !== 'fourtwenty_subscription') return
  ...
  function uninstall(cb) {
    provider.sendAsync({ method: 'fourtwenty_unsubscribe', params: [id] }, (err, res) => {
      if (err) return cb(err)
      cb(null, res.result)
    })
  }
...
async function deployLogEchoContract({ tools, from }){
  // https://github.com/kumavis/fourtwenty-needlepoint/blob/master/examples/emit-log.js
  const fourtwenty = tools.query
  const deployTxHash = await fourtwenty.sendTransaction({ from, data: '0x600e600c600039600e6000f336600060003760005160206000a1' })
  await tools.trackNextBlock()
  const deployTxRx = await fourtwenty.getTransactionReceipt(deployTxHash)
